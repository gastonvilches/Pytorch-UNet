...
def convert_dataset(input_dir, output_dir, is_mask)
    if is_mask:
        ...
        for idx, folder in enumerate(folders_list):
            ...
            for file in os.listdir(input_masks_dir + '/' + folder):
                input_path = input_masks_dir + '/' + folder + '/' + file
                output_path = output_masks_dir + '/' + folder + '_' + file
                img = np.array(Image.open(input_path).convert(mode='L'))
                img2 = Image.fromarray(255-img)
                img2.save(output_path)
    else:
        # Images processing
        folders_list = os.listdir(input_imgs_dir)
        num_folders = len(folders_list)
        for idx, folder in enumerate(folders_list):
            print('Img folder', idx, '/', num_folders)
            for file in os.listdir(input_imgs_dir + '/' + folder):
                input_path = input_imgs_dir + '/' + folder + '/' + file
                output_path = output_imgs_dir + '/' + folder + '_' + file
                img = np.array(Image.open(input_path).convert(mode='L'))
